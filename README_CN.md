...
3. **道具图标不在安装包内**。首次保存**正确的** API Key 后，应用会弹窗引导下载：点**立即下载图标**（约 2 分钟，公共资源不消耗积分），或随时到 设置 → **同步全部图鉴与图片**。图标保存在手机本地，**软件更新不会删除**，仅卸载时清除；未下载时以名称 + emoji 显示
...
| 环境 | 结果 |
|---|---|
| Android 模拟器（Android 37.1）——12 轮功能测试 | 通过 |
| 真机（Android 16 / API 36）——真实 API，约 10 积分 | 通过，0 崩溃（`com.roco.merchant`） |
| `adb reboot` 后不打开应用 | 通过——后台任务自动恢复 |
| 卸载审计（任务/白名单/通知渠道/数据目录） | 通过——彻底清除 |
| 图鉴同步（2500+ 件，43 页） | 通过——0 失败；名称图鉴内置离线，图标免费下载本地缓存 |
