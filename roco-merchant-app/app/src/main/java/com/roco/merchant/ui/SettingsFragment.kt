    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnBatteryExempt.setOnClickListener {
            ...
            Toast.makeText(
                ...
                else "未能自动打开，请在系统设置 → 应用 → 魔力果雷达 → 电池中允许后台运行",
                Toast.LENGTH_LONG
            ).show()
        }
    ...
    private fun showVendorGuide() {
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setMessage(
                ...
                "设置 → 应用设置 → 应用管理 → 魔力果雷达 → 省电策略 → 无限制；\n" +
                "应用信息页开启「自启动」；最近任务里下拉锁定卡片。\n\n" +
                "【华为 / 荣耀（HarmonyOS·EMUI·MagicOS）】\n" +
                "设置 → 应用 → 应用启动管理 → 魔力果雷达 → 手动管理 → 允许自启动/后台活动。\n\n" +
                "【OPPO / 一加（ColorOS）】\n" +
                "设置 → 应用管理 → 魔力果雷达 → 允许自启动 + 允许后台运行；\n" +
                ...
                "设置 → 应用 → 魔力果雷达 → 电池 → 不受限制。\n\n" +
                "【原生 Android】\n" +
                "设置 → 应用 → 魔力果雷达 → 电池 → 不受限制（本页「忽略电池优化」按钮即为快捷入口）。\n\n" +
